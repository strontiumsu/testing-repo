...
class Beamline461(EnvExperiment):
    ...
    def build(self):
       ...
       #     Scannable(default=[NoScan(120*1e6), RangeScan(100*1e6, 140*1e6, 10, randomize=False)],scale=1e6,
        ...
        self.f_MOT3D_load=self.get_dataset('blue_MOT.f_load')
    ...
    def shift_MOT2D_aom_frequency(self,df): 
        ...
        fMOT2D = self.MOT2D_AOM_frequency + df*1e6
        urukul_ch =self.urukul_meas[0]
        dds_ftw_MOT2D_AOM=self.urukul_meas[0].frequency_to_ftw(fMOT2D)
    ...
    def shift_MOT3DDP_aom_frequency(self, df): 
        ...
        fMOT3DDP = self.MOT3DDP_AOM_frequency+df*1e6
        urukul_ch =self.urukul_meas[1]
        dds_ftw_MOT3DDP_AOM=self.urukul_meas[1].frequency_to_ftw(fMOT3DDP)
        urukul_ch.set_mu(dds_ftw_MOT3DDP_AOM, asf=urukul_ch.amplitude_to_asf(self.MOT3DDP_dds_scale)) 
