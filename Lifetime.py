...
class Lifetime(EnvExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument("Rabi_drive_time",
            NumberValue(0.33*1e-6,min=0.0*1e-6,max=20.00*1e-6,scale = 1e-6,
                      unit="us"),"Floppin")    
        
        self.setattr_argument("SE_delay",
            Scannable(default=[RangeScan(0.0*1e-6, 50.0*1e-6, 20, randomize=False),NoScan(0.0)],scale=1e-6,
                      unit="us"),"Floppin")
        ...
        if (not hasattr(self.SE_delay,'sequence') and hasattr(self.Rabi_pulse_freq,'sequence')):
            self.t0=self.SE_delay.value
            self.x=self.Rabi_pulse_freq.sequence
            self.f0=self.x[0]
            print(self.f0)
            self.tscan=False
            
        elif (hasattr(self.SE_delay,'sequence') and not hasattr(self.Rabi_pulse_freq,'sequence')):
            self.f0=self.Rabi_pulse_freq.value
            self.x=self.SE_delay.sequence
            self.t0=self.x[0]
            print(self.f0)
            self.tscan=True
    ...
    def run(self):
        ...
        for ii in range(len(self.x)):
            ...
            if self.Beam3_on: 
                ...
                delay(self.Rabi_drive_time)  # Delay
                self.th_ph.switch3_off()
