...
class Prepare3P0(Scan1D, TimeFreqScan, EnvExperiment):
    ...
    def measure(self, time, frequency):
        ...
        if self.scan == 'frequency':   
            self.ThPh.set_AOM_phase('688_shelf', frequency , 0.0, self.t0, 0)
        ...
        self.MOTs.rMOT_beam_pulse(5*ms)
        ...
        #self.ThPh.AOMs_on(['688_shelf'])
        #delay(18*ms) # wait for field to ramp up
        #self.ThPh.threePhoton_pulse(5*ms) #drive 689
        #self.MOTs.AOMs_on(['3P0_repump','3P2_repump'])
        #self.ThPh.AOMs_off(['688_shelf'])
