...
class Bragg_ARP_Seq_VRS_Sq_exp(Scan1D, TimeFreqScan, EnvExperiment):
    ...
    def measure(self, time, frequency):
        ...
        # Scan the cavity probe for atoms in 0hk########################################
        ...
        # Scan the cavity probe after mom transfer ############################################
        self.MOTs.AOMs_on(['Probe'])# turn probe beam on
        self.MOTs.dac_0.write_dac(3, 5.0) #trigger scan
        self.MOTs.dac_0.load()
        ...
        with parallel:
            ...
            with sequential:
                delay(1*ms)
                self.MOTs.AOMs_off(['Probe']) #turn off probe beam
                self.MOTs.dac_0.write_dac(3, 0.0) # reset cavity trig
                self.MOTs.dac_0.load()
                delay(25*us)
                
        # Scan the cavity probe after squeeze ################################################
        self.MOTs.AOMs_on(['Probe'])# turn probe beam on
        self.MOTs.dac_0.write_dac(3, 5.0) #trigger scan
        self.MOTs.dac_0.load()
