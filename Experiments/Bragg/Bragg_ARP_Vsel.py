...
class Bragg_ARP_Vsel_exp(Scan1D, TimeFreqScan, EnvExperiment):
    ...
    def build(self, **kwargs):
        ...
        #self.setattr_argument("ramsey",BooleanValue(False),"parameters")
    ...
    def prepare_freq_const(self, t):
        ...
        self.core.break_realtime()
        delay(10 * ms)
        self.scan_dds.set(self.Bragg.freq_Bragg1, amplitude=self.Bragg.scale_Bragg1)
        delay(1 * ms)
    ...
    def measure(self, time, frequency):
        ...
        self.prepare_freq_ramp(self.pulse_duration) #Scan Bragg 1 about 100MHz
        delay(1 * ms)



        #self.pulse_duration = self.step_size*(1024*4*ns) # updates the true pulse time due to discretization errors
        #delay(1 * ms)
        ...
        self.MOTs.dac_0.write_dac(4,pulse_time) #set bragg pulse amplitude
        self.MOTs.dac_0.load()
        delay(1*ms)

        self.Bragg.set_AOM_freqs([('Homodyne',self.Bragg.freq_Homodyne)]) # adjust for Doppler

        self.Bragg.set_AOM_freqs([("Bragg2",self.Bragg.freq_Bragg2)]) #adjust frequency of Bragg 2 to select nth-order
        #self.Bragg.set_AOM_freqs([("Bragg2",freq)]) #adjust frequency of Bragg 2 to select nth-order
        delay(1 * ms)
        ...
        with parallel:
            ...
            #self.Bragg.set_AOM_freqs([('Bragg2',self.MOTs.freq_Probe)]) # adjust for Doppler

        #self.Bragg.set_AOM_attens([("Homodyne",3.0)])
        #self.Bragg.set_AOM_freqs([('Homodyne',self.Bragg.freq_Homodyne-0.1*MHz)]) # adjust for Doppler
        #delay(3*ms)
