...
class Blue_MOT_init_dataset(EnvExperiment):
    ...
    def build(self):
         ...
         # 2D MOT
         self.setattr_argument("MOT2D_loading_frequency", NumberValue(194*1e6,min=10.0*1e6,max=200.00*1e6,scale=1e6,
                          unit="MHz"),"2D MOT") 
         self.setattr_argument("MOT2D_attenuation", NumberValue(11.5,min=1.0,max=30.0),"2D MOT")
    ...
    def run(self):
         self.core.reset()
         self.set_dataset("blue_MOT.f_load3D", self.MOT3D_loading_frequency, broadcast=True)
         self.set_dataset("blue_MOT.f_detect", self.MOT3D_detection_frequency, broadcast=True)
         self.set_dataset("blue_MOT.attenuation3D",self.MOT3D_attenuation, broadcast=True)
         self.set_dataset("blue_MOT.amplitude_scale3D",self.MOT3D_DDS_amplitude_scale, broadcast=True)
         
         self.set_dataset("blue_MOT.f_load2D", self.MOT2D_loading_frequency, broadcast=True)
         self.set_dataset("blue_MOT.attenuation2D",self.MOT2D_attenuation, broadcast=True)
         self.set_dataset("blue_MOT.amplitude_scale2D",self.MOT2D_DDS_amplitude_scale, broadcast=True)
         
         self.set_dataset("blue_MOT.f_loadZeeman", self.Zeeman_loading_frequency, broadcast=True)
