...
class Three_photon_Ramsey(EnvExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument("Rabi_pulse_freq",
            Scannable(default=[NoScan(85.0*1e6),RangeScan(60.0*1e+6, 80.0*1e+6, 20, randomize=False)],scale=1e+6,
                      unit="MHz"),"Ramsey")
        
        self.setattr_argument("Beam1_on",BooleanValue(False),"Ramsey")
        self.setattr_argument("Beam2_on",BooleanValue(False),"Ramsey")
        self.setattr_argument("Beam3_on",BooleanValue(True),"Ramsey")
        
        
        self.tscan=False
        self.x=[]
        
        if (not hasattr(self.Ramsey_t_delay,'sequence') and hasattr(self.Rabi_pulse_freq,'sequence')):
            self.t0=self.Ramsey_t_delay.value
            self.x=self.Rabi_pulse_freq.sequence
            self.f0=self.x[0]
            self.tscan=False
            
        elif (hasattr(self.Ramsey_t_delay,'sequence') and not hasattr(self.Rabi_pulse_freq,'sequence')):
    ...
    def run(self):
        ...
        for ii in range(len(self.x)):
           ...
           # BACKGROUND IMAGE SEQUENCE
            ...
            with parallel:
                with sequential:
                    delay(1498.75*us+tdelay+2*self.Rabi_t_pi)
                    with parallel:
                        self.BR.repumpers_on() # turn on repumpers
