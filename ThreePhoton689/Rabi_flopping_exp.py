...
#         initial_guess = [1, 2, 0.5, 3] # [omega (MHz), omega_gen (MHz), s.s., tau]
...
#         self.set_dataset("Gen. Freq (MHz) [cov]", str(pcov[1][1]), broadcast=True)
#         self.set_dataset("Decay (us) [cov]", str(pcov[3][3]), broadcast=True)
...
#             ax.text(t[-1]*0.7, 0.85, f'$\Omega^\prime = 2\pi \times ${np.round(omega_gen,/(2*np.pi)2)} MHz')
...
# def RabiFunc(t, omega, omega_gen, ss, tau):
#     return ss + np.e**(-t/tau)*(omega**2/(2*omega_gen**2)*(1-np.cos(omega_gen*t))-ss)   