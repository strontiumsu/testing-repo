...
class AOM461_camera(EnvExperiment):
    def build(self):
        ...
        # Camera
        self.setattr_argument("t_exposure",NumberValue(20e-3,min=10e-3,max=120e-3,unit="ms",scale=1e-3),"Camera")
        ...
        self.urukul_hmc_ref_probeDP = self.get_device("urukul0_ch1")
        self.urukul_hmc_ref_MOT3D = self.get_device("urukul0_ch2")
        ...
        self.urukul_meas = [self.get_device("urukul0_ch0"),self.get_device("urukul0_ch1"),self.get_device("urukul0_ch2"),self.get_device("urukul0_ch3"),self.get_device("urukul1_ch0")]
    ...
    def run(self):
        ...
        #for iatten in atten:
           ...
           # self.adc_0.set_gain_mu(0,0)
            ...
            # dds_ftw_probeDP=self.urukul_meas[1].frequency_to_ftw(fprobeDP)
            ...
            dds_ftw_MOT3D=self.urukul_meas[2].frequency_to_ftw(fMOT3D)
                ...
                for ii in range(len(self.ProbeDP_AOM_frequency.sequence)):
                    ...
                    with parallel: 
                            with sequential:
                                ...
                                delay(2*ms)
                                urukul_ch.set_mu(dds_ftw_probeDP, asf=urukul_ch.amplitude_to_asf(self.probeDP_dds_scale))
                                self.urukul_hmc_ref_ZeemanDP.sw.off()
                                self.urukul_hmc_ref_MOT2D.sw.off()
